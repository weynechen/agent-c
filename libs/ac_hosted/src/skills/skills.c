 ...
 */
static arc_err_t skill_load_content(ac_skill_t *skill) {
    ...
    arc_err_t err = skill_parse_frontmatter(file_content, &temp_meta, &body_start);
    if (err != ARC_OK) {
        free(file_content);
        return err;
    }
...
arc_err_t ac_skills_discover(ac_skills_t *skills, const char *skill_dir) {
    if (!skills || !skill_dir) {
        return ARC_ERR_INVALID_ARG;
    }
    ...
    arc_err_t err = skill_parse_frontmatter(file_content, &meta, &body_start);
    if (err != ARC_OK) {
        free(file_content);
        return err;
    }
    ...
    if (ac_skills_find(skills, meta.name)) {
        ...
        free(file_content);
        return ARC_OK; /* Not an error, just skip */
    }
...
arc_err_t ac_skills_discover_dir(ac_skills_t *skills, const char *skills_dir) {
    ...
    if (!dir) {
        ...
        return ARC_OK; /* Not an error if directory doesn't exist */
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (ac_skills_discover(skills, subdir_path) == ARC_OK) {
            discovered++;
        }
...
arc_err_t ac_skills_enable(ac_skills_t *skills, const char *name) {
    if (!skills || !name) {
        return ARC_ERR_INVALID_ARG;
    }
    ...
    arc_err_t err = skill_load_content(skill);
    if (err != ARC_OK) {
        return err;
    }
...
arc_err_t ac_skills_disable(ac_skills_t *skills, const char *name) {
    if (!skills || !name) {
        return ARC_ERR_INVALID_ARG;
    }
...
size_t ac_skills_enable_all(ac_skills_t *skills) {
    ...
    while (skill) {
        if (ac_skills_enable(skills, skill->meta.name) == ARC_OK) {
            count++;
        }
        skill = skill->next;
    }
...
arc_err_t ac_skills_validate_tools(
...
arc_err_t ac_skills_set_script_executor(
    ac_skills_t *skills,
